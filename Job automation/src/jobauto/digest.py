...
def _fmt_job(j: dict, idx: int) -> str:
    ...
    posted_age = age_days(j.get("posted_at", ""))
    a = posted_age if posted_age is not None else age_days(j.get("first_seen", ""))
    if a is not None:
        tags.append("today" if a <= 0 else f"{a}d ago")
    else:
        tags.append("date?")
    if posted_age is None and a is not None:
        tags[-1] += " (first seen)"
...
def build_report(db: DB, top_n: int = 15, min_score: int = 50,
    ...
    md.append("")
    if not shortlist:
        md.append("_No matches above threshold._")
